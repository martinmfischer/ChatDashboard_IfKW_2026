...
read_user <- function(i) {
  ...
  admin    <- Sys.getenv(paste0("USER", i, "_ADMIN"), unset = "FALSE")
  
  # Konvertiere "TRUE"/"FALSE" String in logischen Wert
  admin <- toupper(admin) == "TRUE"
...
admin_ui <- fluidPage(
  h3("Admin Panel – Datenspende Dateien"),
  tableOutput("file_table"),
  downloadButton("download_all", "Alle RDS-Dateien herunterladen"),
  
  hr(),
  
  h3("Admin Panel – Nutzerverwaltung"),
  uiOutput("admin_users")  # <- Hier wird die dynamische Passwortverwaltung eingeblendet
)

# Define UI for ChatDashboard application
main_ui <- fluidPage(theme  = shinytheme("flatly"),
...
server <- function(input, output, session) {
  ...
  ################################### SHINYMANAGER / ADMIN PANEL ####
  
  # 1️⃣ Authentifizierung mit User/Password
  res_auth <- secure_server(
    check_credentials = check_credentials(credentials)
  )
  
  # 2️⃣ Prüfen, ob angemeldeter User Admin ist
  is_admin <- reactive({
    req(res_auth$user)
    
    user_row <- credentials[credentials$user == res_auth$user, ]
    if (nrow(user_row) == 1) {
      isTRUE(user_row$admin)
    } else {
      FALSE
    }
  })
  
  # 3️⃣ UI dynamisch je nach Rolle
  output$app_ui <- renderUI({
    req(res_auth$user)
    
    if (is_admin()) {
      admin_ui   # Admin-Oberfläche
    } else {
      main_ui    # Normale Benutzer-Oberfläche
    }
  })
  
  ###########################
  # 4️⃣ RDS-Dateien – Admin Panel
  file_dir <- "./UserData"
  
  files <- reactive({
    list.files(file_dir, pattern = "\\.rds$", full.names = FALSE)
  })
  
  # Tabelle der RDS-Dateien
  output$file_table <- renderTable({
    req(is_admin())  # nur für Admins
    data.frame(Datei = files(), stringsAsFactors = FALSE)
  })
  
  # Download aller RDS-Dateien als ZIP
  output$download_all <- downloadHandler(
    filename = function() {
      paste0("datenspende_", Sys.Date(), ".zip")
    },
    content = function(file) {
      req(is_admin())  # Sicherheitscheck
      
      tmp <- tempfile(fileext = ".zip")
      
      # Wechsel temporär in UserData
      old_wd <- setwd(file_dir)
      on.exit(setwd(old_wd), add = TRUE)  # stellt altes WD wieder her
      
      zip::zip(zipfile = tmp, files = files())  # files() liefert nur Dateinamen ohne Pfad
  ...
  # Reactive DataFrame aller User
  all_users <- reactive({
    req(is_admin())
    credentials
  })
  
  # Dynamische UI: Passwortfelder + Speichern-Buttons
  output$admin_users <- renderUI({
    req(is_admin())
    df <- all_users()
    
    lapply(seq_len(nrow(df)), function(i) {
      fluidRow(
        column(3, strong(df$user[i])),
        column(4, passwordInput(inputId = paste0("pwd_", i), label = "Neues Passwort")),
        column(2, actionButton(inputId = paste0("save_", i), label = "Speichern")),
        column(3, textOutput(outputId = paste0("status_", i)))
      )
    })
  })
  
  # observeEvent für jeden Save-Button
  observe({
    req(is_admin())
    df <- all_users()
    ...
    for (i in seq_len(nrow(df))) {
      local({
        ii <- i  # lokale Kopie für Shiny scoping
        btn_id <- paste0("save_", ii)
        pwd_id <- paste0("pwd_", ii)
        status_id <- paste0("status_", ii)
        
        observeEvent(input[[btn_id]], {
          new_pwd <- input[[pwd_id]]
          if (nzchar(new_pwd)) {
            # Update credentials DataFrame
            credentials$password[ii] <<- new_pwd
            
            # Update .env file
            env_line <- paste0("USER", ii, "_PASSWORD=", new_pwd)
            env_file <- "./credentials.env"
            
            # Alte Zeile ersetzen oder anhängen
            env_lines <- readLines(env_file)
            idx <- grep(paste0("^USER", ii, "_PASSWORD="), env_lines)
            if (length(idx) == 1) {
              env_lines[idx] <- env_line
            } else {
            ...
            # Rückmeldung an UI
            output[[status_id]] <- renderText("✔ Gespeichert")
          } else {
            output[[status_id]] <- renderText("⚠ Passwort leer")
          }
        })
      })
    }
  ...
  # # 8️⃣ Optional: Debugging, wer eingeloggt ist
  # observe({
  #   req(res_auth$user)
  #   cat("USER:", res_auth$user, "\n")
  #   cat("IS ADMIN:", is_admin(), "\n")
  # })
  ...
  observeEvent(c(input$shinyalert,rv$copy,input$show_vars), {
    ...
    if (req(input$shinyalert) == TRUE) {
      ...
      rv$copy2_encrypted <- rv$copy2 ## ripping out encryption for education purposes
